<template>
    <div class="content-wrapper">
        <div class="content-body">
            <b-card>
                <b-card-header header-bg-variant="dark" header-text-variant="white">In Training Search </b-card-header>
                <b-card-body class="border">
                    <b-form>
                        <b-row>
                            ...
                            <b-col md="4">
                                <b-button type="submit" id="basic_sub_btn" @click="traningSearch" class="btn btn-dark shadow mr-1 mb-1">Search</b-button>
                            </b-col>
                        </b-row>
            ...
            <b-card>
                <b-card-header header-bg-variant="dark" header-text-variant="white">Pending In Training For Approval</b-card-header>
                <b-card-body class="border">
                    <b-form
    ...
    export default {
            ...
            return{
                ...
                tableData: {
                    fields: [
                        ...
                        {key: 'training_institute', label: 'Institution Name', sortable: true},
                        {key: 'training_name', label: 'Training Name', sortable: true},
                        {key: 'training_acheivment', label: 'Acheivment', sortable: true},
                        {key: 'training_institute', label: 'Institution Name', sortable: true},

                        {key:'training_start_date', formatter: value => {
                                if(value) {
                                    return moment(value).format('DD-MM-YYYY');
                                }
                            }, sortable:true, class:'text-right'},
                        {key:'training_completion_date',formatter: value => {
                                if(value) {
                                    return moment(value).format('DD-MM-YYYY');
                                }
        ...
        watch:{
            selectedEmployee: function (val, oldVal) {
                if (val !== null) {
                    this.inChargeSearch.emp_id = val.emp_id;
                    this.inChargeSearch.emp_code = val.emp_code;
                    this.inChargeSearch.emp_name = val.emp_name;
                    this.inChargeSearch.department_id = val.dpt_department_id;
                }
            }
        },
        methods: {
            traningSearch() {
                let empId = this.inChargeSearch.emp_id;
                ApiRepository.callApi(ApiRepository.GET_COMMAND, `/pmis/unapproved-traning-list?emp_id=`+empId).then(res => {
            ...
            changeStatus(item) {
                ...
                ApiRepository.callApi(ApiRepository.GET_COMMAND, '/pmis/employee/nominees/update-traning-hold-yn/'+item.nominee_id+'/'+status, items).then(res => {
                    if (res.data.status) {
            ...
            onProcess() {
                ...
                let empId = this.inChargeSearch.emp_id;
                ApiRepository.callApi(ApiRepository.PUT_COMMAND, `/pmis/approve-traning?emp_id=`+empId, obj).then(res => {
