<template>
    <div class="content-wrapper">
        <div class="content-body">
            <b-card>
                <b-card-header header-bg-variant="dark" header-text-variant="white">Address Search </b-card-header>
            ...
            <b-card>
                <b-card-header header-bg-variant="dark" header-text-variant="white">Pending Address For Approval</b-card-header>
                <b-card-body class="border">
    ...
    export default {
            ...
            return{
                ...
                tableData: {
                    fields: [
                        ...
                        {key: 'address_type', label: 'Address Type', sortable: true},
                        {key: 'address_line_1', label: 'Address Line', sortable: true},
                        {key: 'geo_division_name', label: 'Division', sortable: true},
                        {key: 'geo_district_name', label: 'District', sortable: true},
                        {key: 'geo_thana_name', label: 'Thana', sortable: true},
        ...
        methods: {
            changeStatus(item) {
                ...
                ApiRepository.callApi(ApiRepository.POST_COMMAND, '/pmis/update-address-temp/'+item.emp_address_id+'/'+status).then(res => {
            ...
            onProcess() {
                ...
                console.log(obj);
                ApiRepository.callApi(ApiRepository.POST_COMMAND, `/pmis/approve-address`, obj).then(res => {
                    if (res.data.o_status_code == 1) {
                        this.$notify({group: 'pmis', text: res.data.o_status_message, type: 'success'});
                        this.loadAddress()
                    } else {
            ...
            loadAddress() {
                ApiRepository.callApi(ApiRepository.POST_COMMAND, `/pmis/unapproved-address-list`, this.formData).then(res => {
