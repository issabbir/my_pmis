...
<script>
    ...
    export default {
        ...
        data() {
            ...
            return {
                fields: [
                    ...
                    {key: 'effective_date', label:'Effective From', sortable: true},
                    {key: 'previous_basic', label:'Previous Basic', sortable: true, class:'text-right'},
                    {key: 'changed_basic', label:'Changed Basic', sortable: true, class:'text-right'},
                    {key: 'current_month_basic', label:'Current Month Basic', sortable: true, class:'text-right'},
                    ],
                items: [],
                totalList: 1,
                currentPage: 1,
        ...
        methods: {
            ...
            loadData: function () {
                if (this.hasEmpId()) {
                    ApiRepository.callApi(ApiRepository.GET_COMMAND, `/pmis/employee/increments/specific/${this.id}`).then(res => {
                        this.items = res.data;
