        ...
        <b-row  class="border mb-1 pt-1 pb-1" v-if="billStatusShow">
            <b-col>
                ...
                <b-modal v-model="confirmShow" :centered="true" title="Please Confirm" size="sm"
                         ...
                         @ok="billDisbursement()">
                    <div>Are you sure want to disburse the salary?</div>
                </b-modal>
            </b-col>
        </b-row>
    </div>
    ...
    export default {
        ...
        methods: {
            ...
            addState: function(state) {
                ...
                state.fiscal_year = this.process.fiscalYear;
                ApiRepository.callApi(ApiRepository.POST_COMMAND, '/payroll/'+this.process.bill_code+'/status/'+this.process.pr_month, state).then(res => {
            ...
            getBillStatus: function() {
                ApiRepository.callApi(ApiRepository.GET_COMMAND, '/payroll/'+this.process.bill_code+'/status/'+this.process.pr_month).then(res => {
                    this.billStates = res.data.bill_status;
                    this.nextState = res.data.next_status;
                    this.currentStatus = res.data.current_status;
                    this.options = res.data.options;
            ...
            billDisbursement: function() {
                ...
                state.bill_code = this.process.bill_code;
                state.month_id = this.process.pr_month;
                ApiRepository.callApi(ApiRepository.POST_COMMAND, '/payroll/disburse', state).then(res => {
                    if (res.data.o_status_code == 99) {
                        alert(res.data.o_status_message);
                        return;
                    }
