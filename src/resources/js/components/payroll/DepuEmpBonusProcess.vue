...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.$store.commit("setBreadcrumb", {link: "#", label: "Deputation Employee Bonus Process"});
        ...
        methods: {
            ...
            monthWiseSalaryHead(value){
                this.monthId = value;
                    ApiRepository.callApi(ApiRepository.GET_COMMAND, "/payroll/salary-setup/" + value).then(res => {
                            this.bonusHeadObj.noEveryMonthSalary = res.data.years;
                           if(res.data.salaryHeadBasic.length > 0){
           ...
           onFiltered(filteredItems) {
            ...
            processBonus: function() {
                ...
                ApiRepository.callApi(ApiRepository.POST_COMMAND, "/deputation-payroll/monthly-bonus-process", this.bonusHeadObj).then(res => {
                    if (res.data.o_status_code == 1) {
