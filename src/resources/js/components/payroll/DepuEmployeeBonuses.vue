<template>
    <div class="content-wrapper">
        <div class="content-body">
            <b-card>
                <b-card-header header-bg-variant="dark" header-text-variant="white">Deputation Employee Bonus</b-card-header>
                <b-card-body class="border">
                    <b-form @submit.prevent="onSubmit" @reset.prevent="onReset">
                        <b-row>
                            ...
                            <b-col md="3">
                                <b-form-group label="Action" label-for="bill_code">
                                    ...
                                    <b-col md="6" v-if="bonusesHead.bonuses"
                                        ...
                                        <div class="mt-1">
                                            ...
                                            <b-input-group>
                                                ...
                                                <b-form-input class="text-right" v-if="bonus.editable_yn == 'Y' && canHasPermission()"
...
<!--                                        </b-button>-->
                                        <b-button variant="success" @click="emp_bonus()" >
                                        ...
                                        <b-form id="emp_bonus_form" target="_blank" :action="'/report/render/RPT_DEPUTATION_EMPLOYEES_BONUS_REPORT'" method="post">
                                            ...
                                            <input type="hidden" value="/~weblogic/Payroll/RPT_DEPUTATION_EMPLOYEES_BONUS_REPORT.xdo" name="xdo" />
                                            <input type="hidden" value="pdf" name="type" />
                                            <input type="hidden" name="_token" v-model="csrf" />
                                        </b-form>
<!--                                        <b-button v-if="isBillDisburse()" variant="warning" @click="bankAdvise()" >-->

                                        <b-form id="bankadvise_form" target="_blank" :action="'/report/render/RPT_DEPUTATION_BONUS_BANK_PAY_ADVISE'" method="post">
                                            <input type="hidden" :value="process.bill_code" name="p_bill_code" />
                                            ...
                                            <input type="hidden" value="/~weblogic/Payroll/RPT_DEPUTATION_BONUS_BANK_PAY_ADVISE.xdo" name="xdo" />
...
<script>
    ...
    import billStatus from './DepuBillStatus';
    ...
    export default {
        ...
        mounted() {
            ...
            this.$store.commit("setBreadcrumb", {link: "#", label: "Deputation Employee bonus"});
        ...
        methods: {
            editRow(data) {
                ...
                let url = '/deputation-payroll/bonus-heads';
            ...
            load() {
                let url = '/deputation-payroll/salary-process';
            ...
            onSubmit() {
                ...
                this.dataUrl = "/v1/deputation-payroll/bonuses/" + queryString;
                this.isProcessDone = true;

                ApiRepository.callApi(ApiRepository.GET_COMMAND, "/deputation-payroll/bonus-bill-status/"+queryString).then(res => {
            ...
            handleOk(bvModalEvt) {
                let url = '/deputation-payroll/bonus-process/heads';
            ...
            updateBonus: function (bonus) {
                let url = '/deputation-payroll/bonus-process/' + bonus.pr_process_id;
            ...
            addDeduction: function () {
                let url = '/deputation-payroll/pr-bonus-heads/Y';
            ...
            addAllowance: function () {
                let url = '/deputation-payroll/pr-bonus-heads/N';
                ApiRepository.callApi(ApiRepository.POST_COMMAND, url, this.bonus).then(res => {
            ...
            submitDeduction: function () {
                let url = '/deputation-payroll/pr-bonus-heads/Y/add';
            ...
            allBonusHeads() {
                ApiRepository.callApi(ApiRepository.GET_COMMAND, "/deputation-payroll/bonus-process-heads").then(res => {
                    this.bonusHeads = res.data.bonusHeads;
                });
            },
            submitAllowance: function () {
                let url = '/deputation-payroll/pr-bonus-heads/N/add';
            ...
            getMonths: function () {
                ApiRepository.callApi(ApiRepository.GET_COMMAND, "/payroll/depu-bonus-setup/year/" + this.process.fiscalYear).then(res => {
            ...
            getBillCodes:function() {
                ApiRepository.callApi(ApiRepository.GET_COMMAND, "/payroll/depu-bonus-setup/bill-codes/" + this.process.fiscalYear+"/"+ this.process.pr_month).then(res => {
            ...
            deleteHead: function () {
                if (this.selectedHead) {
                    ApiRepository.callApi(ApiRepository.POST_COMMAND, "/deputation-payroll/bonus-process/heads", this.selectedHead).then(res => {
                        if (res.data.params.o_status_code == 1) {
                            this.deductions = res.data.prSalaryHeads;
                            this.bonusesHead = res.data.bonusesHead;
                            this.deduction = {};
