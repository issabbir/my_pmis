            ...
            <b-card>
                ...
                <b-card-body class="border">
                    <b-row class="d-flex justify-content-start">
                        <b-col md="4">
                            <b-form-group label="Year" label-for="year" class="requiredField" label-align-md="left">
                                <b-form-select @change="changeYear" v-model="bonusHeadObj.years" :options="yearsList" name="years" text-field="fy_name" value-field="fy_id" v-validate="'required'"></b-form-select>
                                <span :style="errorMessage">{{ errors.first('years') }}</span>
                            </b-form-group>
                        </b-col>
                        <b-col md="4">
                            <b-form-group label="Month" label-for="month" class="requiredField" label-align-md="left">
                        ...
                        <b-col md="4">
                            <b-form-group label="Bonus Head" class="requiredField" label-for="bonus_head" label-align-md="left">
                                <b-form-select v-model="bonusHeadObj.bonus_head" :options="bonusHeads" name="bonus_head" text-field="pension_head" value-field="pension_head_id" v-validate="'required'"></b-form-select>
                        ...
                        <b-col md="4" >
                            ...
                            >
                                <b-form-select v-model="bonusHeadObj.pension_percentage" name="pensionCat"
                                               :options="process_type_List"
                                               label="text"
                                               v-validate="'required'"
                                               >
                                    <span :style="errorMessage">{{ errors.first('pension_percentage') }}</span>
                                </b-form-select>
                            </b-form-group>
                        </b-col>
                    </b-row>
<!--                    <b-row class="d-flex justify-content-center">-->
<!--                        &lt;!&ndash;                            <h3>Pension Process of {{pr_month_id[pensionHeadObj.pr_month_id]}}</h3>&ndash;&gt;-->
<!--                    </b-row>-->
                    <b-row class="d-flex justify-content-center">
...
<!--                    <b-card-body class="border">-->
<!--                        <b-row class="d-flex justify-content-center">-->
<!--                            <h3>Pension Bonus Process</h3>-->
<!--                        </b-row>-->
<!--                        <b-row class="d-flex justify-content-center">-->
<!--                            <button class="btn btn btn-dark shadow mr-1 mb-1 btn-secondary  process-salary" >Process Bonus</button>-->
<!--                        </b-row>-->
<!--                    </b-card-body>-->
<!--                </b-card>-->
<!--                <b-modal id="modal-center" :centered="true" title="Please Confirm" size="sm" buttonSize="sm" okTitle="YES" cancelTitle="NO" footerClass="p-2"  :hideHeaderClose="false"-->
    ...
    export default {
        ...
        data() {
            return {
                ...
                tableData: {
                    fields: [{key:'salaryHead', label:'salary Head', sortable:true}, {key:'years', label:'Years', sortable:true}, {key:'action', sortable:true}, {key:'bank_name_bng', sortable:true}, 'action'],
                    items: [
        ...
        methods: {
            ...
            changeYear(){
                if(this.bonusHeadObj.years){
                    console.log(this.bonusHeadObj.years)
                    ApiRepository.callApi(ApiRepository.GET_COMMAND, "/pension/salary-setup/months-by-year-id/"+ this.bonusHeadObj.years).then(res => {
                        this.monthsList = res.data;
                        console.log(this.monthsList)
                        let that = this;
                        this.monthsList.forEach( month => {
                            if (month.text == moment().format("MMMM")) {
                                this.bonusHeadObj.months = month.value;
                            }
                        });
                    });
                }
            ...
            allBonusHeads() {
                ApiRepository.callApi(ApiRepository.GET_COMMAND, "/pension/bonus-process-heads").then(res => {
                    ...
                    this.yearsList.forEach(function(item) {
                        if (item.current_yn == 'Y') {
                            that.bonusHeadObj.years = item.fy_id;
                            that.changeYear();
                            return;
                        }
                    });
            ...
            monthWiseSalaryHead(value){
                this.monthId = value;
                ApiRepository.callApi(ApiRepository.GET_COMMAND, "/payroll/salary-setup/" + this.monthId).then(res => {
                    this.bonusHeadObj.noEveryMonthSalary = res.data;
                    // console.log(res.data)

                    console.log(this.bonusHeadObj.noEveryMonthSalary)
            ...
            processBonus: function() {
                ApiRepository.callApi(ApiRepository.POST_COMMAND, "/pension/pension-bonus-process", this.bonusHeadObj).then(res => {
                    if (res.data.o_status_code == 1) {
