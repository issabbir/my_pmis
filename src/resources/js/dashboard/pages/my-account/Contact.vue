        ...
        <b-card>
            <b-card-body class="border" >
                <b-form @submit.prevent="onSubmit" @reset.prevent="onReset">
                    <b-row>
                        <b-col md="5" :style="contacts.emp_contacts_id>0?'pointer-events:none':''">
                            <b-form-group label="Contacts Type" label-for="emp_contact_type_id" class="requiredField">
                                <v-select v-model="contactType" :options="contactsList"
                                ...
                                <div :class="{'invalid-feedback':true ,'d-block':$v.contacts.emp_contact_type_id.$error && !$v.contacts.emp_contact_type_id.required}">Contact type is required!</div>
                            </b-form-group>
                        </b-col>
                        <b-col md="5">
                            <b-form-group label="Contact" label-for="emp_contact_info" class="requiredField">
                                <b-form-input
                                    id="a"
                                    v-model.trim="$v.contacts.emp_contact_info.$model"
                                    type="text"
                                    placeholder="" name="emp_contact"> </b-form-input>
                                <div :class="{'invalid-feedback':true ,'d-block':$v.contacts.emp_contact_info.$error && !$v.contacts.emp_contact_info.required}">Contact info is required!</div>
                            </b-form-group>
                        </b-col>
                        <b-col md="2" class="contacts_btn mt-2">
                            <b-button type="submit" id="basic_sub_btn" class="btn btn-dark shadow" >{{submitBtn}}</b-button>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                fields: [
                    ...
                    {key: 'contact_type.emp_contact_type', label: 'Contacts Type', sortable: true, class: 'widthForty'},
                    {key: 'emp_contact_info', label: 'Contact', sortable: true, class: 'widthForty'},
                    {
                    ...
                    //     }
                    // },
                    {key: 'action', label: 'Actions', class: 'widthTen'}
                ],
                items: [],
        ...
        validations: {
            contacts: {
                emp_contacts_id: {},
                emp_contact_type_id:{required},
                emp_contact_info:{required},
        ...
        mounted() {
            ...
            this.id = this.$store.getters.user.emp_id
            this.contacts.emp_id = this.id;
            this.loadData();
        },
        watch: {
            contactType:function (val,oldVal) {
                if(val !== null) {
                    this.contacts.emp_contact_type_id = val.value;
                } else {
           ...
           /* "contacts.emp_contact_type_id": function() {
                let contact_type = this.contactsList.find( row => {
                    return row.emp_contact_type_id == this.contacts.emp_contact_type_id;
                });

                if(contact_type) {
                    let contact_type_name = contact_type.emp_contact_type.toLowerCase();
        ...
        methods: {
            ...
            editRow(id) {
                ...
                ApiRepository.callApi(ApiRepository.GET_COMMAND, `/pmis/employee/contacts/${id}`).then(result => {
                    ...
                    this.contactType = result.data.contact_type;
                });
            },
            deleteRow() {
