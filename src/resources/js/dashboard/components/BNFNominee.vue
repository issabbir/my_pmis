                            ...
                            <b-row>
                                <b-col md="3">
                                    ...
                                    >
                                        ...
                                        <div :class="{'invalid-feedback':true ,'d-block':$v.nominee.emp_family_id.$error && !$v.nominee.emp_family_id.required}">Family is required!</div>                                    </b-form-group>
                                </b-col>
                                ...
                                <b-col md="3">
                                    ...
                                    >
                                        <v-select v-model="relationships" :options="relationshipWithNomineeList"
                                                  ...
                                                  class="uppercase"
                                                  @input="onInputRelation(relationships.value)">
                                            <template #search="{attributes, events}">
                                                <input
                                                    class="vs__search"
                                                    v-bind="attributes"
                                ...
                                <b-col md="3">
                                    <b-form-group
                                        ...
                                        label-for="auth_id_document" class="d-none">
                                        <div class="custom-file b-form-file">
                                            <input type="file" class="custom-file-input" @change="getAuthIdDocument" accept="application/msword, application/pdf, image/png, image/jpg, image/jpeg"  name="nominee_auth_doc" />
...
<!--                                        <v-select v-model="alternateNominee" multiple :options="familyOptions.filter(a=>a.emp_member_name != selectedFamily.emp_member_name)"-->
                                        <v-select v-model="alternateNominee" multiple :options="familyOptions"
                ...
                <b-table small responsive :items="temp_item" :fields="temp_fields">
                    ...
                    <template v-slot:cell(action)="{ item }">
                        <b-button @click="item.edit_yn = 'Y'" variant="warning">Edit</b-button>
                        <b-button @click="onDelete(item.nominee_id)" variant="danger">Delete</b-button>
                    </template>
                    <template v-slot:cell(approved_yn)="{ item, index, field: { key } }">
                        <b-badge class="w-100" v-if="item[key] == 'Y'" variant="success">Approved</b-badge>
                        <b-badge class="w-100" v-else variant="danger">Not Approved</b-badge>
                    </template>
                </b-table>
                <div >Total Percentage: <strong>{{totalPercentage}}</strong></div>
                <b-row>
                    <b-col class="d-flex justify-content-end align-items-end">
                        <b-button @click="finalSubmit" type="button" variant="primary">Submit</b-button>
                    </b-col>
                </b-row>
            </b-card-body>
        </b-card>
        ...
        watch: {
            ...
            temp_item: {
                ...
                handler(val, oldVal){
                    ...
                    this.temp_item.forEach(this.calculatePercentage)
                }
            },
            alternateNominee: {
        ...
        methods: {
            ...
            loadData: function() {
                if(this.hasEmpId()) {
                    ApiRepository.callApi(ApiRepository.GET_COMMAND, `/pmis/employee/nominees/specific/${this.id}`).then(res => {
                        ...
                        this.items = res.data.data.filter(a=> a.nominee_for_id == 3);
            ...
            finalSubmit() {
                if(this.totalPercentage == 100){
                    let currObj = this;
                    ApiRepository.callApi(ApiRepository.POST_COMMAND, "/pmis/employee/nominees/3", this.temp_item).then(result => {
                        if (result.data.o_status_code == 1) {
                            this.$notify({ group: 'pmis', text: result.data.o_status_message, type:'success' })
                            currObj.loadData();
                            currObj.onReset();
