...
class SalaryProcessController extends Controller
{
    ...
    public function list($fYear, $prMonth, $billCode,Request $request) {
        ...
        $prProcess = $prProcess->select(
            ...
            'pr_salary_process.emp_code', 'pr_salary_process.emp_name','pr_salary_process.pr_year', 'pr_salary_process.pr_month_id', 'pr_salary_process.bill_code', 'pr_salary_process.designation_id','pr_salary_process.deputation_yn') //Bill code instead of biller code.
            ...
            ->groupBy('pr_salary_process.deputation_yn')
            ->orderBy("pr_salary_process.emp_code", 'asc');
        $where = [];
        $orWhere = [];
        if ($fYear) {
        ...
        $prProcess=$prProcess->join('pr_salary_heads', function ($join) {
                $join->on('pr_salary_heads.salary_head_id', '=', 'pr_salary_process.salary_head_id')
                    ->where('pr_salary_process.bonus_yn', '=', 'N');
            });

        if ($searchValue) {
            $prProcess = $prProcess->where(function($q) use ($searchValue){
                $q->where(DB::raw('lower(pr_salary_process.emp_code)'), 'like', "%".strtolower(trim($searchValue))."%")
                    ->orWhere(DB::raw('lower(pr_salary_process.emp_name)'), 'like', "%".strtolower(trim($searchValue))."%");
            });
        }

        $data = $prProcess->where($where)->paginate($length);
        //dd(DB::getQueryLog());
...
//        }
//        $salary_process_data=PrSalaryProcess::where('PR_MONTH_ID',$pr_month_id)
//            ->whereIn('bill_code',$bill_code)
//            ->where('bonus_yn','N')
//            ->first();
//        if($salary_process_data){
//            return ["exception" => true, "o_status_code" => 99, "o_status_message" => "SORRY! THIS SALARY ALREADY PROCESS"];
//        }else{
//            return $this->EMPLOYEE_PR_PROCESS($request);
//        }
    }
    ...
    public function EMPLOYEE_PR_PROCESS(Request $request)
    {
        ...
        try {
            ...
            /*$role_key = LPrBillStatus::where('process_step',1)
                ->where('active_yn','Y')->value('role');
            if($role_key){
                $role_id = Role::where('role_key', $role_key)->value('role_id');
                $user_roles = UserRole::where('role_id', $role_id)->get();
                foreach ($user_roles as $user_role){
                    $operator_notification = [
                        "p_notification_to" => $user_role->user_id,
                        "p_insert_by" => Auth::id(),
                        "p_note" => 'Your approval require for salary process.',
                        "p_priority" => null,
                        "p_module_id" => 2,
                        "p_target_url" => "payroll#/employee-salaries"
                    ];
    ...
    private function getUnUsedHeads($deductionYn, Request $request, PrSalaryHeads $salaryHead) {
        ...
        if($visible_scope = $request->get('biller_scope_yn')){
            $where2[] = ['biller_scope_yn', $visible_scope];
        }
    ...
    private function payrollAllHeads(Request $request) {
        ...
        $heads= DB::table('pr_salary_process')
            ...
            'pr_salary_process.pr_process_id', 'pr_salary_process.salary_head', 'pr_salary_process.deduction_yn', 'pr_salary_process.pr_year',DB::raw('to_char(pr_salary_process.amount,9999999.99) as amount'))
            ->join('pr_salary_heads', function ($join) {
                $join->on('pr_salary_heads.salary_head_id', '=', 'pr_salary_process.salary_head_id')
                    ->where('pr_salary_process.bonus_yn', '=', 'N');
            })
           ->where($where)
            ->orderBy('pr_salary_heads.sort_order', 'ASC')
    ...
    {
        $is_deputation=EmployeeDepu::where('emp_code',$request->get("emp_code"))->first();
        try {
            ...
            if($is_deputation){
                DB::executeProcedure("depu_emp_sal_addition_head", $params);
