...
class DepuEmpBonusProcessController extends Controller
{
    ...
    public function getBonusHeads(){
        $bonusHeads=DB::table('pr_salary_heads')->whereIn('salary_head_id',array(3,23,24,43,25,26,27, 72, 78))->get();
        return[
    ...
    public function list($fYear, $prMonth, $head,$billCode,Request $request) {
        ...
        if ($searchValue) {
            $where[] = [DB::raw('lower(pr_salary_process.emp_code)'), 'like', "%".strtolower(trim($searchValue))."%"];
...
//       }else{
    ...
    public function EMPLOYEE_PR_PROCESS(Request $request)
    {
        try {
            ...
            DB::executeProcedure("EMP_DEPU_BONUS_PROCESS", $params);
        }
        catch (\Exception $e) {
            return ["exception" => true, "status" => false, "message" => $e->getMessage()];
        }
    ...
    private function getUnUsedHeads($deductionYn, Request $request, PrSalaryHeads $salaryHead) {
        ...
        return $salaryHead->where($where2)
                          ->whereIn('salary_head_id',array(3, 23,24,25,26,27, 72, 78))
    ...
    private function bonusAllHeads(Request $request) {
        ...
        $heads= DB::table('pr_salary_process')->select('editable_yn','every_month_yn','emp_code', 'pr_month_id', 'emp_name','designation_id','bill_code',
            'pr_process_id', 'salary_head', 'deduction_yn','remarks', 'pr_year',DB::raw('to_char(amount,9999999.99) as amount'))
//            ->whereIn('salary_head_id',[3, 23,24,25,26,27, 72, 78])
    ...
    private function getEmployeeBonus($params) {
        ...
        if($salary_head_id=$params["salary_head_id"]){
            $where[] = ['salary_head_id', '=', $salary_head_id];
        }
        //dd($prProcess->where($where)->orWhere($orWhere)->first());
